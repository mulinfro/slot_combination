...
def ast_export(stm):
    stm.next()
    rule = ast_rule_helper(stm, True)
    params = ast_params(stm)
    intent_slot = ast_intent_slot(stm)
    weight = ast_weight(stm)
...
def ast_rule(stm):
    stm.next()
    rule = ast_rule_helper(stm, False)
    post_func = ast_rule_post_func(stm)
    syntax_cond_assert(is_rule_end(stm), "expected rule end")
...
def ast_atom(stm):
    ...
    body = ast_atom_body(stm)
    ast_same_type_seq(stm, lambda tkn: syntax_check(tkn, ("SEP","SEMI")))
    return {"tp": "ATOM", "val": body }

def ast_plus(stm, tp="ATOM_PLUS"):
    stm.next()
    rule_name = stm.next().val
    var = ast_rule_ele(stm)
    syntax_cond_assert(var["tp"] == "VAR", "atom plus error")
    return {"tp": tp, "var": var }
    
def ast_rule_helper(stm, is_export=False):
    syntax_assert(stm.peek(), "STR", stm.peek().tp)
    rule_name = stm.next().val
    syntax_assert(stm.next(), ("OP", "="), "%s need = here"%rule_name)
    body = ast_rule_body(stm)
    return {"tp": "RULE_BODY", "rule_name": rule_name, "body": body }

def ast_rule_parn_helper(stm):
    body = []
    while True:
        ele = ast_rule_body(stm)
        body.append(ele)
        if not stm.eof():
...
def ast_rule_body(stm):
    vals, op = [], None
    while True:
        ele = ast_compose_rule_ele(stm)
        vals.append(ele)
...
def ast_intent_slot_helper(stm):
    intents, slots = {}, {}
    while not stm.eof(): 
...
def ast_rule_post_func(ori_stm):
...
def ast_intent_slot(stm):
    skip_tkns(stm, ["=>", "request"])
    i_s_tuple = stm.next()
    if i_s_tuple.tp == "PARN":
        return ast_intent_slot_helper(stream(i_s_tuple.val))
    return {}
